...
## 17. Security Headers, Privacy Notice, and Posture Hardening
A security review pass was completed to improve the site's default browser protections, login-session handling, uploaded-file safety, and public privacy documentation.

*   **Shared Security Header Baseline**:
    *   Added `security_headers.php` and included it centrally so PHP pages emit consistent protections.
    *   Added headers for CSP, frame denial, MIME sniffing protection, referrer limits, browser feature restrictions, cross-origin opener isolation, HTTPS HSTS, and `X-Powered-By` removal.
    *   Mirrored the same core header posture in `.htaccess` for Apache-hosted requests.
*   **Session Cookie Hardening**:
    *   Replaced direct `session_start()` calls on authenticated entry points with `cotswold_secure_session_start()`.
    *   PHP session cookies now use strict session mode, `HttpOnly`, `SameSite=Lax`, and `Secure` when the request is HTTPS.
*   **Private Upload Protection**:
    *   Added `uploads/teamsheets/.htaccess` to block direct web access to uploaded teamsheet documents.
    *   Hardened `digital_teamsheet_file.php` so uploaded files are only served after session/club sharing checks and only if the resolved path stays inside the teamsheet upload directory.
*   **Infrastructure and API Hardening**:
    *   `db.php` now supports environment variables for database credentials and admin passwords, with local XAMPP defaults retained for development.
    *   Database connection failures now return a generic `503 Service Unavailable` instead of exposing connection internals to users.
    *   `fetch_sheet.php` now verifies TLS certificates when downloading Google Sheet exports.
    *   `track_action.php` no longer allows wildcard CORS, only accepts POST requests, validates action tokens, and updates counters with prepared statements.
*   **Privacy Policy Refresh**:
    *   Rewrote `privacy.php` to cover public league records, club representative data, digital teamsheet/swimmer information, uploaded documents, audit history, lawful basis, sharing, retention, security, essential session cookies/browser storage, individual rights, and the ICO complaint route.
    *   Clarified that the site does not use analytics, advertising, marketing, or tracking cookies, while authenticated areas may use essential PHP session cookies.
*   **Documentation Updates**:
    *   Updated `README.md` and `WEBSITE_GUIDE.md` with the security baseline, environment-variable names, upload controls, and privacy-policy scope.
