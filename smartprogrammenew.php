...
<body>
    ...
    <div id="controlPanel" class="no-print bg-yellow-50 border-b border-yellow-200 p-6 flex flex-col items-center justify-center gap-4 shadow-sm">
        <div class="text-center w-full">
            <h3 class="text-lg font-bold text-yellow-900">Programme Setup</h3>
            <p class="text-sm text-yellow-800 mb-2">Select programme type.</p>
        </div>
        <div class="flex items-center gap-3 w-full max-w-sm justify-center mb-2">
            <label class="text-yellow-900 text-sm font-bold uppercase tracking-wider">Type:</label>
        ...
        <?php if ($sheetId || $digitalTeamsheetId): ?>
            ...
            <div id="loadedStatus" class="hidden text-center">
                <h3 class="text-lg font-bold text-green-700">Import Complete</h3>
                <p class="text-sm text-green-600">Your teamsheet was successfully loaded.</p>
            </div>
            
            <div id="sheetSelectContainer" class="hidden flex flex-col items-center gap-2 w-full max-w-sm">
                <div id="workbookNameDisplay" class="bg-blue-50 text-blue-800 text-xs font-bold px-3 py-1 rounded-full mb-1 border border-blue-200"></div>
                <label class="font-bold text-blue-900">Select Sheet</label>
                <select id="sheetSelector" class="w-full p-2 border border-blue-300 rounded shadow-sm"></select>
            </div>

            <button onclick="printProgramme()" class="bg-gray-800 text-white px-6 py-2 rounded-full hover:bg-gray-900 transition font-bold flex items-center gap-2 mt-2 hidden" id="printBtn">
                Print Programme
            </button>
            <script>
        ...
        <?php else: ?>
            <div class="text-center border-t border-yellow-200 pt-4 w-full">
                <h3 class="text-lg font-bold text-yellow-900">Step 2: Upload Data</h3>
                <p class="text-sm text-yellow-800 mb-2">Paste your Teamsheet URL.</p>
            </div>
            <input type="text" id="manualUrl" placeholder="Paste Google Sheet URL here" class="block w-full max-w-sm p-2 border border-slate-300 rounded shadow-sm text-slate-800" />
            <button onclick="importManualUrl()" class="bg-blue-600 text-white px-6 py-2 rounded-full hover:bg-blue-700 transition font-bold flex items-center gap-2 mb-2">
                Load URL
            </button>
            ...
            <button onclick="printProgramme()" class="bg-gray-800 text-white px-6 py-2 rounded-full hover:bg-gray-900 transition font-bold flex items-center gap-2 mt-2">
                Print Programme
            </button>
        <?php endif; ?>
    </div>
    ...
    <script>
        ...
        function importAstData(btn) {
            const sheetId = btn.getAttribute('data-sheet-id');
            const originalText = btn.innerHTML;
            btn.innerHTML = 'Loading...';
            // Wait slightly for visual feedback then fetch
            setTimeout(() => {
                fetchAndProcessSheet(sheetId).then(() => {
        ...
        function importManualUrl() {
            ...
            if (match && match[1]) {
                const btn = document.querySelector('button[onclick="importManualUrl()"]');
                const originalText = btn.innerHTML;
                btn.innerHTML = 'Loading...';
                setTimeout(() => {
